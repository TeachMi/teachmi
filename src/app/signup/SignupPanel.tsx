...
export type SignupSearchParams = Record<string, string | string[] | undefined>;
 ...
 */
function resolveGateIntent(params: SignupSearchParams): GateParams | null {
...
/**
 * Shared signup body — booking-intent resolution + optional intent banner +
 * the form. Rendered by both the full `/signup` page and the `(.)signup`
 * intercepting-route modal, so it deliberately excludes page chrome (AppShell
 * vs. modal shell); each caller supplies its own wrapper.
 */
export async function SignupPanel({ params }: { params: SignupSearchParams }) {
  // Role is fixed by the signup entry point: the become-a-tutor page links
  // here with `?role=tutor`; every other entry (booking gate, header) is a
  // student. `runRegister`'s `coerceRole` re-validates and rejects `admin`.
  const signupRole: "student" | "tutor" =
    firstString(params.role) === "tutor" ? "tutor" : "student";
    ...
    <>
      {tutorDisplayName && (
        <div className="mx-auto w-full max-w-md px-6 pt-10">
          <IntentBanner tutorDisplayName={tutorDisplayName} />
        </div>
      )}
      <SignupForm next={next || undefined} role={signupRole} />
    </>
  );
}
